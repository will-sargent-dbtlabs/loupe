...
\`\`\``,
  ...
  },
  {
    ...
    design_rules: [
      ...
      "Add an `@media print { nav { ... } .tab-panel { display: block !important; } }` override that hides the tab nav and force-shows every panel. Printing does not change which radio is checked, so without this override a browser's print or Save-as-PDF output silently contains only whichever tab happened to be open - the same content-loss failure mode as the content-loss pitfall below, just triggered by printing instead of editing.",
    ],
    pitfalls: [
